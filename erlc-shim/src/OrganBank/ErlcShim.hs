...
data CoreFun = CoreFun
  { cfName   :: String   -- ^ Function name (without quotes)
  , cfArity  :: Int      -- ^ Arity
  , cfBody   :: String   -- ^ Raw body text (for effect analysis)
  } deriving (Show)
...
extractOrganIR inputPath = do
  ...
  (ec, _stdout, stderrOut) <- readProcessWithExitCode "erlc"
    ["+to_core", inputPath] ""
  case ec of
    ExitFailure code -> do
    ...
    ExitSuccess -> do
      ...
      case coreText of
        ...
        Just content -> do
          ...
          pure $ Right $ emitOrganIR modName funs
        ...
        (name, '\'':'/':afterSlash) ->
          case reads afterSlash :: [(Int, String)] of
            [(arity, remaining)]
              ...
              , '=' `elem` remaining -> Just (name, arity)
            _ -> Nothing
        _ -> Nothing
    _ -> Nothing
...
-- | Map a function to its OrganIR effect row.
funEffect :: CoreFun -> Text
funEffect fun
  | isEffectful (cfBody fun) =
      "[{\"module\": \"std\", \"name\": {\"text\": \"io\"}}]"
  | otherwise = "[]"

-- | Emit OrganIR JSON from parsed Core Erlang functions.
emitOrganIR :: String -> [CoreFun] -> Text
emitOrganIR modName funs =
  T.unlines
    [ "{"
...
emitFun :: Int -> CoreFun -> Text
emitFun uid fun =
  let name = T.pack (cfName fun)
      eff  = funEffect fun
      arity = cfArity fun
  ...
  in T.unlines
    [ "      {"
    , "        \"name\": {\"module\": " <> jsonStr (T.pack modName) <> ", \"name\": {\"text\": " <> jsonStr name <> ", \"unique\": " <> T.pack (show uid) <> "}},"
  ...
  where
    modName = ""  -- module name in the qualified name is left empty for now
