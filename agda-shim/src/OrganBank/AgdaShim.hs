...
data MAlonzoDef = MAlonzoDef
  { mdName   :: String   -- ^ Cleaned function name
  , mdParams :: [String] -- ^ Parameter names
  , mdBody   :: String   -- ^ Raw body text
  } deriving (Show)
...
extractOrganIR inputPath = do
  ...
  (ec, _stdout, stderrOut) <- readProcessWithExitCode "agda"
...
parseCase s =
  ...
  let afterCase = trim $ fromMaybe s (stripPrefix "case " s)
      ...
      afterCase' = if "coe " `isPrefixOf` afterCase
...
splitBranches s =
  let ls = lines s
      -- Group lines: a branch starts with a non-blank, non-indented line
...
parsePrimOp s = case s of
  _ | "addInt " `isPrefixOf` s -> Just ("add", twoArgs (drop 7 s))
...
-- | Emit complete OrganIR JSON.
emitOrganIR :: String -> FilePath -> String -> [MAlonzoDef] -> Text
emitOrganIR modName srcFile agdaVer defs = T.unlines
  [ "{"
  , "  \"source_language\": \"agda\","
  , "  \"module_name\": " <> jsonStr (T.pack modName) <> ","
  , "  \"source_file\": " <> jsonStr (T.pack srcFile) <> ","
  , "  \"compiler_version\": " <> jsonStr (T.pack agdaVer) <> ","
  , "  \"definitions\": [" <> T.intercalate ",\n" (zipWith emitDef [1..] defs) <> "]"
  , "}"
  ]

emitDef :: Int -> MAlonzoDef -> Text
emitDef uid def =
  let name = mdName def
      params = mdParams def
      arity = length params
      bodyExpr = parseBody def
  in T.unlines
    [ "    {"
    , "      \"name\": {\"module\": \"\", \"text\": " <> jsonStr (T.pack name) <> ", \"unique\": " <> T.pack (show uid) <> "},"
    , "      \"type\": {\"tag\": \"any\"},"
    , "      \"expr\": " <> emitExpr bodyExpr <> ","
    , "      \"sort\": \"fun\","
    , "      \"visibility\": \"public\","
    , "      \"arity\": " <> T.pack (show arity)
    , "    }"
    ]
...
-- | Emit an expression as JSON.
emitExpr :: Expr -> Text
emitExpr (EVar name uid) =
  "{\"tag\": \"var\", \"name\": " <> jsonStr (T.pack name) <> ", \"unique\": " <> T.pack (show uid) <> "}"
emitExpr (EApp name uid args) =
  "{\"tag\": \"app\", \"name\": " <> jsonStr (T.pack name) <> ", \"unique\": " <> T.pack (show uid)
    <> ", \"args\": [" <> T.intercalate ", " (map emitExpr args) <> "]}"
emitExpr (ELet name uid bind body) =
  "{\"tag\": \"let\", \"name\": " <> jsonStr (T.pack name) <> ", \"unique\": " <> T.pack (show uid)
    <> ", \"bind\": " <> emitExpr bind <> ", \"body\": " <> emitExpr body <> "}"
emitExpr (ECase scrut uid branches) =
  "{\"tag\": \"case\", \"scrutinee_name\": " <> jsonStr (T.pack scrut) <> ", \"scrutinee_unique\": " <> T.pack (show uid)
    <> ", \"branches\": [" <> T.intercalate ", " (map emitBranch branches) <> "]}"
emitExpr (ELam params body) =
  "{\"tag\": \"lam\", \"params\": [" <> T.intercalate ", " (map emitParam params) <> "], \"body\": " <> emitExpr body <> "}"
emitExpr (ELitInt n) =
  "{\"tag\": \"lit_int\", \"value\": " <> T.pack (show n) <> "}"
emitExpr (ELitStr s) =
  "{\"tag\": \"lit_str\", \"value\": " <> jsonStr (T.pack s) <> "}"
emitExpr (EPrim op args) =
  "{\"tag\": \"prim\", \"op\": " <> jsonStr (T.pack op) <> ", \"args\": [" <> T.intercalate ", " (map emitExpr args) <> "]}"
emitExpr (ECon name args) =
  "{\"tag\": \"con\", \"name\": " <> jsonStr (T.pack name) <> ", \"args\": [" <> T.intercalate ", " (map emitExpr args) <> "]}"
emitExpr EUnreachable =
  "{\"tag\": \"unreachable\"}"

emitBranch :: Branch -> Text
emitBranch br =
  "{\"con\": " <> jsonStr (T.pack (brCon br))
    <> ", \"binders\": [" <> T.intercalate ", " (map emitBinder (brBinders br))
    <> "], \"body\": " <> emitExpr (brBody br) <> "}"

emitBinder :: (String, Int) -> Text
emitBinder (name, uid) = "[" <> jsonStr (T.pack name) <> ", " <> T.pack (show uid) <> "]"

emitParam :: (String, Int) -> Text
emitParam (name, uid) = "[" <> jsonStr (T.pack name) <> ", " <> T.pack (show uid) <> "]"
