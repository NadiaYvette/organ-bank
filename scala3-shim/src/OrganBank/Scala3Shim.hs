...
data ScalaDef = ScalaDef
  { sdName       :: !Text       -- ^ Method name
  , sdParams     :: ![(Text, Text)]  -- ^ (paramName, paramType) pairs
  , sdReturnType :: !Text       -- ^ Return type
  , sdBody       :: !ScalaExpr  -- ^ Parsed body expression
  , sdVisibility :: !Text       -- ^ "public" or "private"
  }
...
data ScalaExpr
  ...
  | SELitStr !Text
  | SEVar !Text
  | SEApp !ScalaExpr ![ScalaExpr]     -- ^ Function application
  | SEPrim !Text !ScalaExpr !ScalaExpr -- ^ Binary primitive: op, lhs, rhs
  | SEIf !ScalaExpr !ScalaExpr !ScalaExpr  -- ^ if/then/else
  | SELet !Text !ScalaExpr !ScalaExpr      -- ^ val binding
  | SENew !Text ![ScalaExpr]          -- ^ new Constructor(args)
  | SEBlock ![ScalaExpr]              -- ^ { stmts; expr }
  | SEUnparsed !Text                  -- ^ Fallback for unparsed fragments
...
extractOrganIR inputPath = do
  (exitCode, _stdoutText, stderrText) <-
    readProcessWithExitCode scalaBin
...
joinContinuations (l1:l2:rest)
  ...
  where
    isContinuation a b =
      let sa = T.strip a
          sb = T.strip b
      in not (T.null sa) && not (T.null sb)
      ...
      in if depth' == 0
         then case findMatchingOp ops t pos of
                Just (op, opStart) ->
                  ...
                  in if not (T.null lhs) && not (T.null rhs)
...
findMatchingOp (op:rest) t pos =
  let opLen = T.length op
      start = pos - opLen + 1
  in if start >= 0 && T.take opLen (T.drop start t) == op
...
hasParenCall t =
  case T.breakOn "(" t of
    (before, after) ->
      not (T.null before) && not (T.null after) &&
...
parseDotExpr t =
  let (receiver, methodAndArgs) = findTopLevelDot t
      ...
      method = T.takeWhile (\c -> c /= '(') methodAndArgs
  ...
  in if isArithOp method
     ...
     else if method == "box" && isBoxableType receiver
...
emitOrganIR modName srcFile classes =
  let allDefs = concatMap scDefs classes
      -- Filter out compiler-generated defs
      userDefs = filter (not . isCompilerGenerated) allDefs
  in T.unlines
    [ "{"
    , "  \"schema_version\": \"1.0.0\","
    , "  \"metadata\": {"
    , "    \"source_language\": \"scala3\","
    , "    \"compiler_version\": \"scala-3.8.2\","
    ...
    , "    \"definitions\": ["
    , T.intercalate ",\n" (zipWith (emitDef modName) [1..] userDefs)
    , "    ],"
    , "    \"data_types\": [],"
    , "    \"effect_decls\": []"
    , "  }"
...
emitDef :: String -> Int -> ScalaDef -> Text
emitDef modName uid def =
  let name      = sdName def
      params    = sdParams def
      retTy     = sdReturnType def
      vis       = sdVisibility def
      paramArgs = map emitParamType params
      -- Detect IO effect for main methods
      effects   = if name == "main" then [ioEffect] else []
      bodyExpr  = emitExpr (sdBody def)
  in T.unlines
    [ "      {"
    , "        \"name\": {\"module\": " <> jsonStr (T.pack modName)
        <> ", \"name\": {\"text\": " <> jsonStr name
        <> ", \"unique\": " <> T.pack (show uid) <> "}},"
    , "        \"type\": {"
    , "          \"fn\": {"
    , "            \"args\": [" <> T.intercalate ", " paramArgs <> "],"
    , "            \"effect\": {\"effects\": [" <> T.intercalate ", " effects <> "]},"
    , "            \"result\": " <> emitType retTy
    , "          }"
    , "        },"
    , "        \"expr\": " <> emitLam params bodyExpr <> ","
    , "        \"sort\": \"fun\","
...
ioEffect :: Text
ioEffect = "{\"module\": \"std\", \"name\": {\"text\": \"io\"}}"

-- | Emit a lambda wrapping parameters and a body.
emitLam :: [(Text, Text)] -> Text -> Text
emitLam [] body = body
emitLam params body =
  T.concat
    [ "{\"elam\": {\"params\": ["
    , T.intercalate ", " (zipWith emitLamParam [100..] params)
    , "], \"body\": "
    , body
    , "}}"
    ]

emitLamParam :: Int -> (Text, Text) -> Text
emitLamParam uid (name, ty) =
  "{\"name\": {\"text\": " <> jsonStr name
    <> ", \"unique\": " <> T.pack (show uid)
    <> "}, \"type\": " <> emitType ty <> "}"

-- | Emit a parameter's type for the function type signature.
emitParamType :: (Text, Text) -> Text
emitParamType (_, ty) =
  "{\"multiplicity\": \"many\", \"type\": " <> emitType ty <> "}"

-- | Emit a type reference.
emitType :: Text -> Text
emitType ty =
  let mapped = mapScalaType ty
  in "{\"con\": {\"qname\": {\"module\": \"scala\", \"name\": {\"text\": "
       <> jsonStr mapped <> "}}}}"
...
-- | Emit an expression as OrganIR JSON.
emitExpr :: ScalaExpr -> Text
emitExpr (SELitInt n) =
  "{\"elit\": {\"int\": " <> T.pack (show n) <> "}}"
emitExpr (SELitStr s) =
  "{\"elit\": {\"str\": " <> jsonStr s <> "}}"
emitExpr (SEVar v) =
  let (mod_, name) = splitQualName v
  in "{\"evar\": {\"text\": " <> jsonStr name
       <> ", \"unique\": 0"
       <> (if T.null mod_ then "" else ", \"module\": " <> jsonStr mod_)
       <> "}}"
emitExpr (SEApp fn args) =
  "{\"eapp\": {\"fn\": " <> emitExpr fn
    <> ", \"args\": [" <> T.intercalate ", " (map emitExpr args) <> "]}}"
emitExpr (SEPrim op lhs rhs) =
  "{\"eprim\": {\"op\": " <> jsonStr op
    <> ", \"args\": [" <> emitExpr lhs <> ", " <> emitExpr rhs <> "]}}"
emitExpr (SEIf cond thenE elseE) =
  T.concat
    [ "{\"ecase\": {\"scrutinee\": " , emitExpr cond
    , ", \"branches\": ["
    , "{\"pattern\": {\"pat_lit\": {\"bool\": true}}, \"body\": " , emitExpr thenE , "}"
    , ", "
    , "{\"pattern\": {\"pat_lit\": {\"bool\": false}}, \"body\": " , emitExpr elseE , "}"
    , "]}}"
    ]
emitExpr (SELet name bound body) =
  "{\"elet\": {\"name\": {\"text\": " <> jsonStr name <> ", \"unique\": 0}"
    <> ", \"value\": " <> emitExpr bound
    <> ", \"body\": " <> emitExpr body <> "}}"
emitExpr (SENew name args) =
  "{\"eapp\": {\"fn\": {\"evar\": {\"text\": " <> jsonStr name <> ", \"unique\": 0}}"
    <> ", \"args\": [" <> T.intercalate ", " (map emitExpr args) <> "]}}"
emitExpr (SEBlock []) = "{\"elit\": {\"int\": 0}}"
emitExpr (SEBlock [e]) = emitExpr e
emitExpr (SEBlock (e:es)) =
  "{\"elet\": {\"name\": {\"text\": \"_\", \"unique\": 0}"
    <> ", \"value\": " <> emitExpr e
    <> ", \"body\": " <> emitExpr (SEBlock es) <> "}}"
emitExpr (SEUnparsed t) =
  if T.null t
  then "{\"elit\": {\"int\": 0}}"
  else "{\"evar\": {\"text\": " <> jsonStr t <> ", \"unique\": 0}}"

-- | Split a qualified name like "Module.name" into ("Module", "name").
splitQualName :: Text -> (Text, Text)
splitQualName t =
  case T.breakOnEnd "." t of
    (prefix, name)
      | T.null prefix -> ("", t)
      | otherwise     -> (T.dropEnd 1 prefix, name)  -- drop trailing "."
...
jsonStr s = "\"" <> T.concatMap escapeChar s <> "\""
  where
    ...
    escapeChar '\t' = "\\t"
    escapeChar c
      | ord c < 0x20 = "\\u" <> T.justifyRight 4 '0' (T.pack (showHex' (ord c)))
      | otherwise     = T.singleton c
    showHex' n
      | n < 16    = "0" ++ showH n
      | otherwise = showH (n `div` 16) ++ showH (n `mod` 16)
    showH n
      | n < 10    = [toEnum (n + ord '0')]
      | otherwise = [toEnum (n - 10 + ord 'a')]
