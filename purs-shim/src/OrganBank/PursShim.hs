...
coreFnToOrganIR sourceFile coreFnText =
  let modName   = extractJsonString "moduleName" coreFnText
      exports   = extractJsonStringArray "exports" coreFnText
      decls     = extractDecls coreFnText
      defsJson  = T.intercalate ",\n" (zipWith (emitDef modName) [1..] decls)
      exportsJson = T.intercalate ", " (map jsonStr exports)
  in T.unlines
    [ "{"
    , "  \"schema_version\": \"1.0.0\","
    , "  \"metadata\": {"
    , "    \"source_language\": \"purescript\","
    , "    \"source_file\": " <> jsonStr (T.pack sourceFile) <> ","
    , "    \"shim_version\": \"0.1.0\""
    , "  },"
    , "  \"module\": {"
    , "    \"name\": " <> jsonStr modName <> ","
    , "    \"exports\": [" <> exportsJson <> "],"
          ...
          in case T.uncons afterColon of
              ...
              Just ('[', arrRest) ->
                ...
                in case T.breakOn "\"" trimmed of
                    (modPart, _) -> T.intercalate "." $ T.splitOn "\",\"" $
                      T.filter (\c -> c /= ' ' && c /= '\n' && c /= '\r' && c /= '\t') $
...
emitDef :: Text -> Int -> CoreFnDecl -> Text
emitDef modName uniq decl =
  let name   = declIdent decl
      arity  = declArity decl
      sort   = if arity > 0 then "fun" :: Text else "val"
      -- Build the expression: nested lambdas for functions, placeholder for values
      anyTy  = "{\"con\": {\"qname\": {\"module\": \"std\", \"name\": {\"text\": \"any\"}}}}"
      params = T.intercalate ", " $
        [ "{\"name\": {\"text\": " <> jsonStr (T.pack ("$" <> show i)) <> "}, \"type\": " <> anyTy <> "}"
        | i <- [1..arity]
        ]
      bodyExpr = "{\"elit\": {\"int\": 0}}"
      expr   = if arity > 0
        then "{\"elam\": {\"params\": [" <> params <> "], \"body\": " <> bodyExpr <> "}}"
        else bodyExpr
      -- Build the type
      argTypes = T.intercalate ", " $
        [ "{\"multiplicity\": \"many\", \"type\": " <> anyTy <> "}"
        | _ <- [1..arity]
        ]
      fnType = "{\"fn\": {\"args\": [" <> argTypes <> "], \"effect\": {\"effects\": []}, \"result\": " <> anyTy <> "}}"
      ty = if arity > 0 then fnType else anyTy
  in T.intercalate "\n"
    [ "      {"
    , "        \"name\": {\"module\": " <> jsonStr modName <> ", \"name\": {\"text\": " <> jsonStr name <> ", \"unique\": " <> T.pack (show uniq) <> "}},"
    , "        \"type\": " <> ty <> ","
    , "        \"expr\": " <> expr <> ","
    , "        \"sort\": " <> jsonStr sort <> ","
    , "        \"visibility\": \"public\""
...
-- | Escape a Haskell Text to a JSON string literal.
jsonStr :: Text -> Text
