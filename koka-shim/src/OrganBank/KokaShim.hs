...
stripFunVal t =
  let t' = case T.stripPrefix "pub " t of
              Just r  -> r
              Nothing -> t
...
parseEffectRow t =
  let -- Split on | first (polymorphic effect row)
      (concrete, _polyVar) = T.breakOn "|" t
      -- Split on commas
      parts = map T.strip (T.splitOn "," concrete)
  in map mapEffect (filter (not . T.null) parts)

-- | Is this word a known effect name?
isEffectName :: Text -> Bool
isEffectName w = w `elem`
  [ "total", "div", "console", "io", "exn", "raise"
  , "ndet", "alloc", "read", "write", "st", "net", "fsys"
  , "ui", "blocking"
  ]
...
mapType t = case T.strip t of
  ...
  "string"  -> "std.string"
  "()"      -> "std.unit"
  other     -> if isSimpleIdent other then "any" else "any"

-- | Check if something looks like a simple identifier (not a complex type).
isSimpleIdent :: Text -> Bool
isSimpleIdent t = not (T.null t) && T.all (\c -> isAlphaNum c || c == '.' || c == '-' || c == '_' || c == '/') t

------------------------------------------------------------------------
-- JSON emission (hand-rolled to avoid aeson dependency)
------------------------------------------------------------------------

emitJSON :: OrganModule -> Text
emitJSON m = T.pack $ concat
  [ "{\n"
  , "  \"organ\": \"koka\",\n"
  , "  \"module\": ", jsonStr (omName m), ",\n"
  , "  \"imports\": ", jsonStrList (omImports m), ",\n"
  , "  \"definitions\": [\n"
  , intercalate ",\n" (map (emitDef "    ") (omDefs m)), "\n"
  , "  ]\n"
  , "}"
  ]

emitDef :: String -> OrganDef -> String
emitDef pad d = concat
  [ pad, "{\n"
  , pad, "  \"name\": ", jsonStr (odName d), ",\n"
  , pad, "  \"params\": [\n"
  , intercalate ",\n" (map (emitParam (pad ++ "    ")) (odParams d)), "\n"
  , pad, "  ],\n"
  , pad, "  \"effects\": ", jsonStrList (odEffects d), ",\n"
  , pad, "  \"returnType\": ", jsonStr (odReturnType d), "\n"
  , pad, "}"
  ]

emitParam :: String -> (Text, Text) -> String
emitParam pad (n, ty) = concat
  [ pad, "{\"name\": ", jsonStr n, ", \"type\": ", jsonStr ty, "}" ]

jsonStr :: Text -> String
jsonStr t = "\"" ++ escapeJSON (T.unpack t) ++ "\""
...
jsonStrList :: [Text] -> String
jsonStrList xs = "[" ++ intercalate ", " (map (T.unpack . (\t -> "\"" <> escapeJSONT t <> "\"")) xs) ++ "]"

escapeJSONT :: Text -> Text
escapeJSONT = T.concatMap $ \c -> case c of
  '"'  -> "\\\""
  '\\' -> "\\\\"
  '\n' -> "\\n"
  _    -> T.singleton c
